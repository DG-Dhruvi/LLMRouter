...
_PD      = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "profile_data"))
_RESULTS = os.path.join(os.getcwd(), "results")
...
DEFAULT_SAVE_PT          = os.path.join(_RESULTS, "result_data_graph", "standard", "query_task_domain_graph_full.pt")
...
def load_query_data(json_path: str) -> dict[str, list[str]]:
    ...
    Expected format:
        { "dataset_name": ["query text 1", "query text 2", ...], ... }

    Args:
        json_path : path to the queries JSON file

    Returns:
        dict mapping dataset name → list of query strings
    """
    path = Path(json_path)
    if not path.exists():
...
def collect_query_nodes(
    ...
    Args:
        query_data        : { dataset_name: [query, ...] } from load_query_data()
        dataset_name2idx  : dataset name → dataset node index

    Returns:
        query_texts       : list[str]  – node_feature_text for each query node
        query_dataset_ids : list[int]  – dataset node index for each query node
                            (parallel to query_texts)
    """
...
def build_query_edge_indices(
    ...
    Args:
        query_dataset_ids : list[int] of length N_queries, parallel to query nodes.

    Returns:
        Dict with two edge types (both directions):
          "query_to_dataset" : (query_src, dataset_dst, None)
          "dataset_to_query" : (dataset_src, query_dst, None)
    """
...
def encode_all_nodes(
    ...
    """
    Encode feature texts for all five node types using Longformer.

    Returns:
        arch_emb, model_emb, dataset_emb, domain_emb, query_emb  – each FloatTensor [N, D]
    """
    print("\n[1/5] Encoding architecture nodes ...")
    arch_emb = encode_texts(arch_feature_texts)

    print("\n[2/5] Encoding model nodes ...")
    model_emb = encode_texts(model_feature_texts)

    print("\n[3/5] Encoding dataset nodes ...")
    dataset_emb = encode_texts(dataset_feature_texts)

    print("\n[4/5] Encoding domain nodes ...")
    domain_emb = encode_texts(domain_feature_texts)

    print("\n[5/5] Encoding query nodes ...")
    query_emb = encode_texts(query_texts)

    return arch_emb, model_emb, dataset_emb, domain_emb, query_emb
...
def assemble_graph(
    ...
    Node types : architecture, model, dataset, domain, query
    Edge types :
      ...
      query        <-> dataset (membership, no edge attr)
    ...
    # query <-> dataset  (undirected, no edge attr)
...
def main(
    ...
    arch_emb, model_emb, dataset_emb, domain_emb, query_emb = encode_all_nodes(
        arch_feature_texts, model_feature_texts,
        dataset_feature_texts, domain_feature_texts, query_texts,
    )
...
def build_query_task_domain_graph(
    mode:        str        = "standard",
    json:        str | None = None,
    arch:        str | None = None,
    ...
    """
    Build the query-task-domain graph (all 5 node types).
    ...
    main(
        ...
        save_path=save          or os.path.join(_out_dir, "query_task_domain_graph_full.pt"),
    )


def cli() -> None:
    parser = argparse.ArgumentParser(
        description="Build query-task-domain graph (all node types) for RouteProfile."
    )
    ...
    build_query_task_domain_graph(mode=_args.mode, json=_args.json, arch=_args.arch,
                                  dataset=_args.dataset, domain_map=_args.domain_map,
                                  domain_feat=_args.domain_feat, query=_args.query,
                                  save=_args.save)
